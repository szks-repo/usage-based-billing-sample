...
// SchemaMigration represents a row from 'usage_based_billing.schema_migrations'.
...
func (sm *SchemaMigration) Insert(ctx context.Context, db DB) error {
	...
	// insert (manual)
	const sqlstr = `INSERT INTO usage_based_billing.schema_migrations (` +
		`version, dirty` +
		`) VALUES (` +
		`?, ?` +
		`)`
...
func (sm *SchemaMigration) Update(ctx context.Context, db DB) error {
	...
	const sqlstr = `UPDATE usage_based_billing.schema_migrations SET ` +
		`dirty = ? ` +
		`WHERE version = ?`
	// run
...
func (sm *SchemaMigration) Upsert(ctx context.Context, db DB) error {
	...
	const sqlstr = `INSERT INTO usage_based_billing.schema_migrations (` +
		...
		`version = VALUES(version), dirty = VALUES(dirty)`
	// run
	logf(sqlstr, sm.Version, sm.Dirty)
	if _, err := db.ExecContext(ctx, sqlstr, sm.Version, sm.Dirty); err != nil {
		return logerror(err)
...
func (sm *SchemaMigration) Delete(ctx context.Context, db DB) error {
	...
	const sqlstr = `DELETE FROM usage_based_billing.schema_migrations ` +
		`WHERE version = ?`
	// run
...
// SchemaMigrationByVersion retrieves a row from 'usage_based_billing.schema_migrations' as a [SchemaMigration].
//
...
func SchemaMigrationByVersion(ctx context.Context, db DB, version int64) (*SchemaMigration, error) {
	...
	const sqlstr = `SELECT ` +
		`version, dirty ` +
		`FROM usage_based_billing.schema_migrations ` +
		`WHERE version = ?`
	// run
	logf(sqlstr, version)
	sm := SchemaMigration{
